...
def get_compiled_model():
    ...
    model = tf.keras.Sequential([
        # tf.keras.layers.Input(shape=[114]),
...
def fold_evaluation():
    ...
    for code_process_technique in code_process_techniques:
        ...
        for performance_metric in performance_metrics:
            ...
            precision_buffer = []
            recall_buffer = []
            print("DNN model in performance metric data: ", performance_metric)
            ...
            for train_index, test_index in KFold(n_split).split(X):
                ...
                # result_buffer.append('\n')
                print("%s: %.2f%%" % (model.metrics_names[1], precision * 100))
                print("%s: %.2f%%" % (model.metrics_names[2], recall * 100))
                print("%s: %.2f%%" % (model.metrics_names[3], auc * 100))
